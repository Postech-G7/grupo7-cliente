import { EventosPedido, Pedido, StatusPedido } from "../../entities/pedido";
import { ItemPedido } from "../../entities/itemPedido";
import { PedidoVersao } from "../../entities/pedido.versao";
import { PedidoDatabase } from "../../../adapter/driven/infra/database/pedido.database";
import { CustomError } from "../../../../suporte/entities/custom.error";
import { ClienteUseCases } from "../../../../cliente/core/applications/usecases/cliente.usecases";
import { Cliente } from "../../../../cliente/core/entities/cliente";
import { format } from "date-fns";
import { ProdutoUseCases } from "./produto.usecases";
import { PagamentoUseCases } from "../../../../pagamento/core/applications/usecases/pagamento.usecases";
import {
  TipoPagamento,
  Pagamento,
} from "../../../../pagamento/core/entities/pagamento";
...
export class PedidoUseCases {
  ...
  async adiciona(
    ...
    for (let i = 0; i < itens.length; i++) {
      ...
      const produto = await this.produtoService
        .buscaUltimaVersao(item.getCodigoProduto())
        .then();
    ...
    var pedidoVersao: PedidoVersao = await this.database
      .adiciona(pedido)
      .then();
  ...
  private preparaItesPedido(itens: Array<any>) {
    ...
    for (let i = 0; i < itens.length; i++) {
      const { codigoProduto, quantidade, observacao } = itens[i];

      itensPedido.push(new ItemPedido(codigoProduto, quantidade, observacao));
    }
  ...
  async atualizaStatus(
    ...
    const pedidoVersao = await this.alteraStatus(
      cpf,
      this.preparaItesPedido(itens),
      codigoPedido,
  ...
  async buscaStatus(codigoPedido: string): Promise<StatusPedido> {
    ...
    if (ultimaVersao) {
      ...
      throw new CustomError(
        "Pedido não encontrado com o código informado",
        404,
        false,
        []
      );
  ...
  async checkout(codigoPedido: string): Promise<any> {
    ...
    if (pedido.getValorPedido() <= 1) {
      throw new CustomError(
        "Valor do pedido menor do que o permitido",
        400,
        false,
        []
      );
    ...
    const pagamentoDados = {
      ...
      identificadorExterno: pedido.getCodigoPedido()!,
    };

    let pagamentoRequest = new Pagamento(pagamentoDados);
    const response = await this.pagamentoUseCases
      .criar(pagamentoRequest)
      .then();
    const pagamento = await this.pagamentoUseCases
      .buscaUltimaVersao(pedido.getCodigoPedido()!)
      .then();
    ...
    pagamento.setMetadata(response.metadata);
    return {
      pedido: pedidoVersao,
      pagamento,
    };
  ...
  async webhook(codigoPedido: string, evento: string): Promise<any> {
    ...
    if (
      pedido.getStatus() === StatusPedido.Pagamento &&
  ...
  private async baixarPagamentoPedido(pedido: Pedido): Promise<PedidoVersao> {
    const pagamento: Pagamento = await this.pagamentoUseCases
      .buscaUltimaVersao(pedido.getCodigoPedido()!)
      .then();
