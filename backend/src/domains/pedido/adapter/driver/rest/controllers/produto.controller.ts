    ...
    try {
      return next(
        new CustomResponse(
          200,
          "Produto adicionado",
          await this.service.adiciona(
            new Produto(codigo, produto, categoria, preco, descricao)
          )
        )
      );
    } catch (err) {
    ...
    try {
      ...
      return next(
        new CustomResponse(
          200,
          "Produto adicionado",
          await this.service.buscaUltimaVersao(codigo)
        )
      );
    } catch (err) {
      return next(
    ...
    try {
      ...
      if (categoria) {
        return next(
          new CustomResponse(
            200,
            "Busca de produto por categoria",
            await this.service.buscaCategoria(categoria)
          )
        );
      } else {
        return next(
          new CustomResponse(
            200,
            "Listagem de produto",
            await this.service.buscaListaProduto()
          )
        );
      }
    } catch (err) {
