...
import { ItemPedido } from "../../../../core/entities/itemPedido";
    ...
    try {
      ...
      return next(
        new CustomResponse(
          201,
          "Pedido adicionado",
          await this.service.adiciona(cpf, this.preparaItesPedido(itens))
        )
      );
    } catch (err) {
      console.error(err);
      return next(
        new CustomError("Ops, algo deu errado na operação", 500, false, err)
      );
    }
  }
    ...
    try {
      ...
      next(
        new CustomResponse(
          ...
          await this.service.atualizaStatus(codigoPedido, statusPedido)
        )
      );
    } catch (err) {
      console.error(err);
    ...
    try {
      ...
      next(
        new CustomResponse(
          200,
          "Status do Pedido",
          await this.service.buscaStatus(codigoPedido)
        )
      );
    } catch (err) {
      next(
    ...
    try {
      ...
      next(
        new CustomResponse(
          200,
          "Pedido encontrado",
          await this.service.buscaUltimaVersao(codigoPedido)
        )
      );
    } catch (err) {
      next(
    ...
    try {
      next(
        new CustomResponse(
          200,
          "Pedido encontrado",
          await this.service.listaPedidos()
        )
      );
    } catch (err) {
      next(
    ...
    try {
      ...
      next(
        new CustomResponse(
          201,
          "Webhook Processado",
          await this.service.webhook(codigoPedido, evento)
        )
      );
    } catch (err) {
      next(
    ...
    try {
      ...
      next(
        new CustomResponse(
          200,
          "Pedido recebido",
          await this.service.checkout(request.params.codigoPedido)
        )
      );
    } catch (err) {
      console.error(err);
